...
void MetisGraphPartitioner::doPartition(const Table & table, int & num_partitions, Array<int> & partitioning)
{
	if(table.Size() != table.Width())
...
void MetisGraphPartitioner::doPartition(const SparseMatrix & wtable, int & num_partitions, Array<int> & partitioning)
{
	if(wtable.Size() != wtable.Width())
		mfem_error("Table is not square!");

	int num_vertices(wtable.Size());
	int num_edges(wtable.NumNonZeroElems());
...
void MetisGraphPartitioner::doPartition(const SparseMatrix & wtable, const Array<int> & vertex_weight, int & num_partitions, Array<int> & partitioning)
{
	if(wtable.Size() != wtable.Width())
		mfem_error("Table is not square!");
...
void MetisGraphPartitioner::doPartition(const SparseMatrix & wtable, const Vector & vertex_weight, int & num_partitions, Array<int> & partitioning)
{
	if(wtable.Size() != wtable.Width())
		mfem_error("Table is not square!");
	...
	doPartition(i, j, edge_weight, vertex_weight_int, num_partitions, partitioning);
}

void MetisGraphPartitioner::doRecursivePartition(const SparseMatrix & wtable, const Array<int> & vertex_weight, Array< int > & num_elements, Array < Array<int> *> & partitioning)
{
	if(wtable.Size() != wtable.Width())
		mfem_error("Table is not square!");
	...
	for(k=1; k < number_of_recursions; ++k)
	{
		   ...
		   edgecut = doPartition(i, j, edge_weight, vertex_weight, num_elements[number_of_recursions-k], *part);
		   partitioning[number_of_recursions-k-1] = part;
		   comparePartitioning(num_elements[number_of_recursions+1-k], *(partitioning[number_of_recursions-k]), num_elements[number_of_recursions-k], *(partitioning[number_of_recursions-k-1]) );
	}
}
...
void comparePartitioning(int nCoarsePart, const Array<int> & coarsePart, int & nFinePart, Array<int> & finePart)
{
	...
	std::cout << "The number of fine partitions has increased from " << nFinePart << " to " << iCF[nCoarsePart] << "\n";
...
// Private funtions
//=======================================================================
int MetisGraphPartitioner::doPartition(const Array<int> & i, const Array<int> & j, const Array<int> & edge_weight,
										const Array<int> & vertex_weight, int & num_partitions, Array<int> & partitioning)
{
   ...
   else
   {
      ...
      else
      {
    	  ...
    	  {
    		  std::cout<<"edge_weights is of size "<< edge_weight.Size() << "It should be" << num_edges <<"\n";
    		  throw 1;
    	  }
      }
      ...
      {
    	  if(vertex_weight.Size() == num_vertexes)
    		  vertex_weight_ptr = const_cast<int *>(vertex_weight.GetData() );
    	  else
    	  {
    		  std::cout<<"vertex_weights is of size "<< vertex_weight.Size() << "It should be" << num_vertexes <<"\n";
    		  throw 2;
    	  }
      }
      ...
      if( flags & REORDER)
      {
    	  ...
    	  {
    		  std::cout << " we can not sort edges if edge_weight is provided! \n";
    		  throw 3;
    	  }
      }
#endif
...
int CheckPartitioning(const Table & table, const int num_vertexes, const int * const partitioning)
{
   ...
   if (n_empty > 0)
   {
      std::cout << "The following partitions are empty :\n";
      for (i = 0; i < num_comp.Size(); i++)
         if (num_comp[i] == 0)
            std::cout << ' ' << i;
      std::cout << std::endl;
...
void build_partitioning_from_table(int nrows, int ncols, const int * table_i, const int * table_j, Array<int> & partitioning )
{
	...
	{
		for(const int * jcol(table_j+table_i[i]); jcol < table_j+table_i[i+1]; ++jcol)
			partitioning[*jcol] = i;
	}
}
//======================================================================================//
...
void MetisGraphPartitioner::doPartition(const Table & table, int num_vertexes, int & num_partitions, int * partitioning)
{
	if( table.Width() != num_vertexes )
		mfem_error("MetisGraphPartitioner::doPartition ");
   ...
   {
      const int *Iconst(table.GetI()), *Jconst(table.GetJ());
      int *I, *J;
      I = const_cast<int *>(Iconst);
      J = const_cast<int *>(Jconst);
