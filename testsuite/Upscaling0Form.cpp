...
//
//
// Sample runs:  mpirun -np 4 ./Upscaling0Form --datafile=data.getpot
//               mpirun -np 4 ./Upscaling0Form --datafile=data2D.getpot
...
int main (int argc, char *argv[])
{
	...
	dynamic_cast<DeRhamSequenceFE *>(sequence[0])->ReplaceMassIntegrator(AgglomeratedTopology::ELEMENT, 0, new MassIntegrator(coeffH1), false);
	dynamic_cast<DeRhamSequenceFE *>(sequence[0])->ReplaceMassIntegrator(AgglomeratedTopology::ELEMENT, 1, new VectorFEMassIntegrator(coeffDer), true);
	...
	for(int k(0); k < nLevels; ++k)
	{
		...
		const SharingMap & h1_dofTrueDof( sequence[k]->GetDofHandler(0)->GetDofTrueDof() );

		Vector prhs( h1_dofTrueDof.GetTrueLocalSize() );
		h1_dofTrueDof.Assemble(*(rhs[k]), prhs);
		HypreParMatrix * pA = Assemble(h1_dofTrueDof, *A, h1_dofTrueDof);
		...
		//ADS Solver
		{
			...
			h1_dofTrueDof.Distribute(psol, *(sol_ADS[k]));
