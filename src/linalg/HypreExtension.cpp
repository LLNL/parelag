...
{
	G = seq->ComputeTrueDerivativeOperator(0);
	if(data.cycle_type > 10 )
		seq->ComputeTrueProjectorFromH1ConformingSpace(1, Pix, Piy, Piz);
	else
		Pi = seq->ComputeTrueProjectorFromH1ConformingSpace(1);
...
HypreADS::HypreADS(HypreParMatrix &A, HypreParMatrix &G_, HypreParMatrix &C_, Array<HypreParMatrix *> & ND_Pi_, Array<HypreParMatrix *> & RT_Pi_, const HypreADSData & data):
	HypreSolver(&A),
	G(&G_),
	C(&C_),
	ND_Pi(NULL),
...
HypreADS::HypreADS(HypreParMatrix &A, HypreParMatrix &G_, HypreParMatrix &C_, HypreParMatrix &ND_Pi_, HypreParMatrix &RT_Pi_):
	HypreSolver(&A),
	G(&G_),
	C(&C_),
	ND_Pi(&ND_Pi_),
...
HypreADS::HypreADS(HypreParMatrix &A, HypreParMatrix &G_, HypreParMatrix &C_, HypreParMatrix &ND_Pi_, HypreParMatrix &RT_Pi_, const HypreADSData & data):
	HypreSolver(&A),
	G(&G_),
	C(&C_),
	ND_Pi(&ND_Pi_),
...
#ifdef DERHAMSEQUENCE_HPP_
HypreADS::HypreADS(HypreParMatrix &A, DeRhamSequence * seq, const HypreADSData & data):
		HypreSolver(&A),
		G(NULL),
		C(NULL),
		ND_Pi(NULL),
		...
		owns_Pi(1)
{
	...
	G = seq->ComputeTrueDerivativeOperator(0);
	C = seq->ComputeTrueDerivativeOperator(1);

    if(data.dataAMS.cycle_type > 10 )
    	seq->ComputeTrueProjectorFromH1ConformingSpace(1, ND_Pix, ND_Piy, ND_Piz);
    else
    	ND_Pi = seq->ComputeTrueProjectorFromH1ConformingSpace(1);

    if(data.cycle_type > 10 )
       	seq->ComputeTrueProjectorFromH1ConformingSpace(2, RT_Pix, RT_Piy, RT_Piz);
    else
    	RT_Pi = seq->ComputeTrueProjectorFromH1ConformingSpace(2);
...
void HypreADS::SetParameters( const HypreADSData & data )
{
   ...
   HYPRE_ADSSetAMSOptions(ads, data.dataAMS.cycle_type, data.dataAMS.dataAlpha.coarsen_type, data.dataAMS.dataAlpha.agg_levels,
		   data.dataAMS.dataAlpha.relax_type, data.dataAMS.dataAlpha.theta, data.dataAMS.dataAlpha.interp_type, data.dataAMS.dataAlpha.Pmax);
}
