...
std::vector<int> RedistributeElements(
   ParallelCSRMatrix &elem_face, int &num_redist_procs, bool use_geometric_partitioning)
{
   ...
   if (elem_face.NumRows() > 0)
   {
      ...
      else
      {
         ...
         partition.SetSize(globProc_globProc.Height());
         partitioner.Partition(partition.Size(), num_redist_procs, partition);
      }
...
Redistributor::Redistributor(const AgglomeratedTopology& topo, int& num_redist_procs, bool use_geometric_redistribution)
   : redTrueEntity_trueEntity(topo.Codimensions()+1),
     redEntity_trueEntity(topo.Codimensions()+1),
     redTrueDof_trueDof(topo.Dimensions()+1),
     redDof_trueDof(topo.Dimensions()+1)
{
   auto elem_redist_procs = RedistributeElements(topo.TrueB(0), num_redist_procs, use_geometric_redistribution);
   Init(topo, elem_redist_procs);
}
   ...
   const AgglomeratedTopology& topo, const std::vector<int>& elem_redist_procs)
{
   ...
   if (topo.Conn_[1])
   {
      auto elem_trueRidge = IgnoreNonLocalRange(
               *(topo.entityTrueEntity[0]), *(topo.GetConnectivity(AgglomeratedTopology::ELEMENT, AgglomeratedTopology::RIDGE).AsCSRMatrix()), *(topo.entityTrueEntity[2]));
   ...
   if (topo.Conn_[2])
   {
      auto elem_truePeak = IgnoreNonLocalRange(
               *(topo.entityTrueEntity[0]), *(topo.GetConnectivity(AgglomeratedTopology::ELEMENT, AgglomeratedTopology::PEAK).AsCSRMatrix()), *(topo.entityTrueEntity[3]));
...
Redistributor::Redistribute(const AgglomeratedTopology& topo)
{
   ...
   if (topo.Codimensions() > 1)
   {
      ...
      auto redB = RAP(*redEntity_trueEntity[1], trueB1, *TE_redE);
   ...
   if (topo.Codimensions() > 2)
   {
      ...
      auto redB = RAP(*redEntity_trueEntity[2], trueB2, *TE_redE);
