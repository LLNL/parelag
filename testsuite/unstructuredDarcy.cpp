...
int main (int argc, char *argv[])
{
	...
	int coarseningFactor = data("coarseningFactor", 8);
	std::string meshfile = data("mesh", "mesh.mesh3d");
	int feorder = data("feorder", 0);
	int upscalingOrder = data("upscalingorder", 0);
	int aggressiveLevels = data("aggressiveLevels", 1);

	//Overload options from command line
	ser_ref_levels = cmm_line("--nref_serial",   ser_ref_levels);
	par_ref_levels = cmm_line("--nref_parallel", par_ref_levels);
    meshfile = cmm_line("--meshfile", meshfile.c_str() );
	feorder = cmm_line("--feorder", feorder);
	upscalingOrder = cmm_line("--upscalingorder", upscalingOrder);
	coarseningFactor = cmm_line("--coarseningFactor", coarseningFactor);
	aggressiveLevels = cmm_line("--aggressiveLevels", aggressiveLevels);
	...
	partitioner.doPartition(mesh->ElementToElementTable(), mesh->GetNE(), num_procs, partitioning);

	ParMesh * pmesh = new ParMesh(comm, *mesh, partitioning);
	delete mesh;
	...
	for(int ilevel = 0; ilevel < nLevels-1; ++ilevel)
	{
		...
		partitioner.doPartition( *(topology[ilevel]->LocalElementElementTable()), topology[ilevel]->Weight(AgglomeratedTopology::ELEMENT), level_NE[ilevel+1], partitioning );
	...
	for(int k(0); k < nLevels; ++k)
	{
		...
		if(myid == 0)
			std::cout << "Timing ELEM_AGG_LEVEL " << k << ": Assembly mass matrices done in " << tdiff << "s. \n";
	...
	for(int k(0); k < nLevels; ++k)
	{
		...
		tdiff = chrono.RealTime();
		if(myid == 0)
			std::cout << "Timing ELEM_AGG_LEVEL " << k << ": Assembly Darcy Operator and Preconditioner done in " << tdiff << "s. \n";
		timings(k, ASSEMBLY) += tdiff;

		ndofs[k] = pM->GetGlobalNumRows() + pB->GetGlobalNumRows();
		nnzs[k] = pM->NNZ() + pB->NNZ(); // could multiply B by 2, but don't think that's actually the appropriate measure
