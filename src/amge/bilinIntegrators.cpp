...
SpectralLumpedIntegrator::SpectralLumpedIntegrator(BilinearFormIntegrator * _bfi, int _own_bfi):
		bfi(_bfi),
		own_bfi(_own_bfi)
{
...
void SpectralLumpedIntegrator::AssembleElementMatrix(const FiniteElement &el, ElementTransformation &Trans, DenseMatrix &elmat)
{
	int nd = el.GetDof();
	elmat.SetSize(nd);
	elmat = 0.;
...
DiagonalIntegrator::DiagonalIntegrator(BilinearFormIntegrator * _bfi, int _own_bfi):
		bfi(_bfi),
		own_bfi(_own_bfi)
{
