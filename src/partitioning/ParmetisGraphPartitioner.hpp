...
public:
    ...
    *                      vertex v belongs to partition p. (OUT).
	 ...
	 */
	int doPartitionEqualWeight(ParallelCSRMatrix & table, int & num_partitions, Array<int> & partitioning);
    ...
    *                      vertex v belongs to partition p. (OUT).
	 ...
	 * returns number of (LOCAL) partitions actually created (TODO: just returns requested)
	 */
	int doPartition(ParallelCSRMatrix & wtable, int & num_partitions, Array<int> & partitioning);
    ...
    *                      vertex v belongs to partition p. (OUT).
	 ...
	 * returns number of (LOCAL) partitions actually created (TODO: just returns requested)
	 */
	int doPartition(ParallelCSRMatrix & wtable, const Array<int> & vertex_weight, 
						 int & num_partitions, Array<int> & partitioning);

	int doPartition(ParallelCSRMatrix & table, const Array<int> & edge_weight,
