...
int main (int argc, char *argv[])
{
	...
	//PROGRAM OPTIONS
	int nref_parallel = data("nref_parallel", 4);
	...
	if(myid == 0)
		std::cout << "Refine mesh in parallel "   <<   nref_parallel << " times. \n";
...
//	Element::Type type = Element::TETRAHEDRON;
	...
	Mesh *mesh = new Mesh(n, n, n, type, 1);

	int nDimensions = mesh->Dimension();

	int n_el_c = mesh->GetNE();
	...
	int nLevels = nref_parallel+1;
	Array<int> level_nElements(nLevels);
	for (int l = 0; l < nref_parallel; l++)
	{
		level_nElements[nref_parallel-l] = pmesh->GetNE();
	...
	Array<HypreParMatrix *> allC(nLevels);
	...
	Array<HypreParMatrix *> allP_bc(nLevels-1);
	Array<HypreParMatrix *> allC_bc(nLevels);
...
//			HypreSmoother prec(*pA, HypreSmoother::Jacobi);
			...
			hdiv_dofTrueDof.Distribute(psol, *(sol_ADS[k]));
...
//	DenseMatrix timings(nLevels-1, NSTAGES);
	...
	std::cout << "\n{\n";
	int w = 14;
	std::cout << "%level" << std::setw(w) << "size" << std::setw(w) << "nit AMGe" << std::setw(w) << "nit ADS"
			<< std::setw(w) << "Assembly" << std::setw(w) << "Prec AMGe" << std::setw(w) << "Prec ADS"
			<< std::setw(w) << "Solver AMGe" << std::setw(w) << "Solver ADS\n";
	for(int i(0); i < nLevels; ++i)
		std::cout<< i << std::setw(w) << ndofs[i] << std::setw(w) << iter(i,AMGe)<< std::setw(w) << iter(i,ADS)
