...
int main (int argc, char *argv[])
{
    ...
    else if (solve_form == 1)
    {
        auto DRSequence_FE = sequence[0]->FemSequence();
        FiniteElementSpace * ufespace = DRSequence_FE->GetFeSpace(uform); // H(curl)
        FiniteElementSpace * pfespace = DRSequence_FE->GetFeSpace(pform); // H(div)

        mfem::LinearForm bform(ufespace);
        Vector source_vec(3);
        source_vec = 1.;
        Vector vectorofzeros(3);
        vectorofzeros = 0.;
        VectorConstantCoefficient source(source_vec);
        VectorConstantCoefficient fbdr3d(vectorofzeros);
        bform.AddDomainIntegrator(new VectorFEDomainLFIntegrator(source));
        bform.AddBoundaryIntegrator(new VectorFEBoundaryTangentLFIntegrator(fbdr3d));
        ...
        *rhs_u = 0.0;
        ...
        mfem::Vector prhs;
        {
            ...
            auto A = ToUnique(Add(*M, *RAP(*D, *W, *D)));
        ...
        if (visualize)
        {
            ...
            MultiVector p(tmp.GetData(), 1, tmp.Size());
            ...
            {
                hierarchy.ShowTrueData(start_level, hierarchy.GetRedistributionIndex(start_level), groupid, uform, u);
                hierarchy.ShowTrueData(start_level, hierarchy.GetRedistributionIndex(start_level), groupid, nDimensions, p);
            }
        }
