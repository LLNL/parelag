...
int main(int argc, char **argv)
{
    ...
    {
        if (myrank == 0)
            std::cerr << "slud_test only runs for 8 or fewer procs. "
    ...
    *par_X = 0.0;
    ...
    slu.Factor();
    slu.Mult(*par_B,*par_X);

    par_A->Mult(*par_X,*par_B,-1.0,1.0);

    size_t num_passes = 0;
    if (par_B->Norml2() < 1e-10) ++num_passes;

    if (!myrank)
        std::cout << "Test 1:"
                  << (par_B->Norml2() < 1e-10 ? "Success!" : "Failure. :(")
                  << '\n';
    // {
    //     for (int ii = 0; ii < par_X->Size(); ++ii)
    //         std::cout << (*par_X)(ii) << std::endl;
    ...
    mfem::Vector myB(par_B->Size()),myX(par_X->Size());
    for (int ii = 0; ii < my_num_rows; ++ii)
        myX(ii) = x_values[ii];
    ...
    if (myB.Norml2() < 1e-10) ++num_passes;
    if (!myrank)
        std::cout << "Test 2:"
