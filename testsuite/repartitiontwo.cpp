 ...
 *
 * eventually want to use parmetis here
 *
...
int main (int argc, char *argv[])
{
   ...
   delete b;
	...
	int nparts = num_procs / 2;
	ParmetisGraphPartitioner partitioner;
	Array<int> partitioning(fespace->TrueVSize());
	partitioner.doPartitionEqualWeight(*A, nparts, partitioning);

	ProcessorCoarsening pcoarse(comm);
	if (myid == 0) std::cout << "Reducing from " << num_procs << " to " 
									 << num_procs / 2 << " processors." << std::endl;
	pcoarse.SetUpReorder(partitioning, num_procs / 2, rowpart);

	// should probably do it again back the other way and check matrix equality...
